...
class TestStudentUpdateForm(TestCase):
	...
	def setUp(self):
		...
		self.url = reverse('students_edit', kwargs={'pk': '2'})
	...
	def test_form(self):
		...
		self.assertIn('name="cancel_button_student_update', response.content)
	...
	def test_cancel(self):
		...
		response = self.client.post(self.url, {'cancel_button_student_update':'Cancel'}, follow=True)

		
		self.assertIn('Editing of student has been cancelled!', response.content)
		self.assertEqual(response.redirect_chain[0][0],
			'http://testserver/?status_message=' + 
			'Editing%20of%20student%20has%20been%20cancelled!')
	...
	def test_access(self):
		...
		self.assertEqual(response.redirect_chain[0], 
			('http://testserver/users/login/?next=/students/2/edit/', 302))

	def test_styles(self):
		#login as admin to access students edit form 
		self.client.login(username='admin', password='admin')
