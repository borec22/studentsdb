...
class JournalView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
    	...
    	# check if we need t odisplay some specific month
    	if self.request.GET.get('month'):
    		month = datetime.strptime(self.request.GET['month'], \
    			    '%Y-%m-%d').date()
    	else:
    		# othetwise just displaying current month data
    	...
    	# prepare variable for template to genarate
        ...
        # apply pagination, 10 students pe page
    ...
    def post(self, request, *args, **kwargs):
        ...
        # get or create journal object for given student andmonth
        journal = MonthJournal.objects.get_or_create(student=student, date=month)[0]

        # set new presence on journal for given student and save resylt
